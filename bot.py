...
COOLDOWN_SECONDS = 8
DAILY_LIMIT = 30                      # Максимум 30 скачиваний в сутки на пользователя
# ====================================================
...
# Статистика и лимиты
user_stats = {}      # user_id {count int, reset_time datetime}
user_cooldowns = {}
...
def download_media(url str, output_dir Path, is_audio bool = False)
    if is_audio
        ...
        ydl_opts = {
            outtmpl str(output_dir  %(title)s.%(ext)s),
            format bestvideo[height=1080][filesize400M]best[height=1080]best,
            noplaylist True,
            quiet False,
            retries 5,
            socket_timeout 40,
...
async def start(message Message)
    await message.answer(
        📥 bSaveReelBotbnn
        Отправь ссылку на пост из Instagram, TikTok или YouTube.n
        Поддержка видео • фото • карусели • аудио (MP3)nn
...
async def stats(message Message)
    ...
    await message.answer(
        f📊 bВаша статистикаbnn
        fСкачано сегодня b{count}b  {DAILY_LIMIT}n
        fЛимит обновится завтра.,
        parse_mode=HTML
    )
...
async def handle_url(message Message)
    ...
    try
        is_audio = audio in url.lower() or any(x in url for x in [music, mp3])  # простой способ
        ...
        if file_path.endswith('.mp3')
            ...
            await message.answer_audio(audio=audio, caption=✅ Готово! @SaveReelBot)
        else
            media = FSInputFile(file_path)
            await message.answer_document(media=media, caption=✅ Готово! @SaveReelBot)  # document лучше работает с фото+каруселями

        await status.delete()

        logger.info(fUser {user_id} downloaded {url})

    except Exception as e
        error = str(e).lower()
        logger.error(fError for user {user_id} {e})
        if any(word in error for word in [private, unavailable, login])
            await status.edit_text(❌ Видеофото приватное или недоступно.)
